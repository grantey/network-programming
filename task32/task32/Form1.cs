...
namespace task32
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + "\\task32.cfg";
            try
            {
                LoadCfg(path);
            }
        ...
        {
            FileStream fs = File.Open("task32.cfg", FileMode.Create, FileAccess.Write);
        ...
        private void Listening(object o)
        {
            ...
            while (true)
            {
                if (TCP.Pending())
                {
                    ...
                    t.IsBackground = true;
                    td = new ThreadData(TCP.AcceptSocket(), rule.extport);
                    t.Start(td);
                }
            }
        ...
        private void Execute(object o)
        {
            ...
            if (td.socket.Connected)
            {
                ...
                /* попытка жестко перенаправлять трафик */
                /*
                Regex myReg = new Regex(@"Host: (((?<host>.+?):(?<port>\d+?))|(?<host>.+?))\s+", RegexOptions.Multiline | RegexOptions.IgnoreCase);
                Match m = myReg.Match(System.Text.Encoding.ASCII.GetString(httpRequest));
                string host = m.Groups["host"].Value;
                ...
                string before = System.Text.Encoding.ASCII.GetString(httpRequest);
                string after = before.Replace(host, "81.19.70.3");
                int cook;
                if ((cook = after.IndexOf("Cookie")) > 0) after = after.Remove(cook) + "\r\n";

                httpRequest = Encoding.ASCII.GetBytes(after);

                IPEndPoint point = new IPEndPoint(IPAddress.Parse("81.19.70.3"), 80);

                FileStream fs = File.Open("ttt.txt", FileMode.Append, FileAccess.Write);
                StreamWriter stream = new StreamWriter(fs);
                try
                {
                      string s = System.Text.Encoding.ASCII.GetString(httpRequest);
                      stream.WriteLine(s);
                }
                finally
                {
                ...
                using (Socket rerouting = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
                {
                    ...
                    else
                    {
                      ...
                      /*  FileStream fs1 = File.Open("qqq.txt", FileMode.Append, FileAccess.Write);
                        StreamWriter stream1 = new StreamWriter(fs1);
                        try
                        {
                            string s = System.Text.Encoding.ASCII.GetString(httpResponse);
                            stream1.WriteLine(s);
                        }
                        finally
                        {
    ...
    public struct ThreadData
    {
        ...
        public ThreadData (Socket s, int i)
        {
            socket = s;
            port = i;
        }
